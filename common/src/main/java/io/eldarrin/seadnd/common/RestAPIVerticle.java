...
public class RestAPIVerticle extends BaseMicroserviceVerticle {
     ...
     */
    protected Promise<HttpServer> createHttpServer(Router router, String host, int port) {
        ...
        retriever.getConfig(res -> {
            if (res.succeeded()) {
                ...
                createHttpServer(router, host, port).future()
                        .compose(serverCreated -> publishHttpEndpoint(serviceName, service, port, serviceType).future())
                        .result();
            } else {
    ...
    protected void addHealthHandler(Router router, Promise<Void> promise) {
